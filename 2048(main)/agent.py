...
class nTupleNetwork:
    def __init__(self, tuples):
        self.TUPLES = tuples
        self.TARGET_PO2 = 15  #target power of 2
        self.LUTS = self.initialize_LUTS(self.TUPLES)

    def initialize_LUTS(self, tuples):
        LUTS = [] # stands for Look Up TableS
        for tp in tuples:
            LUTS.append(np.zeros((self.TARGET_PO2 + 1) ** len(tp))) # append a arr of 16 to power len(tp) zeros
        return LUTS
    ...
    def evaluate(self, s, a):
        ...
        b = Board(s)
        try:
            r = b.act(a) # what happens after doing action a
            s_after = b.copyboard()
    ...
    def best_action(self, s):
        ...
        a_best = None
        r_best = -1
        for action in [UP, RIGHT, DOWN, LEFT]: # we evaluate reward values for all possible actions in the given state s,
            r = self.evaluate(s, action)
            if r > r_best:
                r_best = r #then choose the action that results in highest reward value
                a_best = action
        return a_best

    def learn(self, s, a, r, s_after, s_next, alpha=0.01, debug=False): #alpha means the learning rate
